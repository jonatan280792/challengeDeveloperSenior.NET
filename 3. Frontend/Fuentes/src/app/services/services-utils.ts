...
export class ServiceUtils {
  ...
  public buildRequest(endpoint, method, data?, isTextResponse?) {
    const headers = Config.api.options;

    if (endpoint.needsAuth) {
      headers.headers
        = headers.headers.set('token', this.sessionService.getToken());
    } else {
      headers.headers = headers.headers.delete('token');
    }
